...
export class ReconciliationManager {
   ...
   */
  reconcile(serverPosition, sequence, collided) {
    ...
    // If difference is significant or a collision occurred, reconcile
    if (distance > this.reconciliationThreshold || collided) {
      this.reconciliationCount++;

      if (this.debugEnabled) {
        console.log(
          `Reconciliation needed: client=(${clientPosition.x.toFixed(
            2
          )}, ${clientPosition.y.toFixed(
            2
          )}), server=(${serverPosition.x.toFixed(
            2
          )}, ${serverPosition.y.toFixed(2)}), diff=${distance.toFixed(2)}${
            collided ? ", collision detected" : ""
          }`
        );
      }
